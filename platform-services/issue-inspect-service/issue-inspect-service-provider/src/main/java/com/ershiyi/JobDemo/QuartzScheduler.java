...
public class QuartzScheduler {
    ...
    private void startJob(Scheduler scheduler) throws SchedulerException {
        ...
        // JobDetail 是具体Job实例
        JobDetail jobDetail = JobBuilder.newJob(QuartzJobListener.class).withIdentity("知识点And题目", "推送类型").build();
        // 基于表达式构建触发器
        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule("0 0/5 * * * ? ");
        // CronTrigger表达式触发器 继承于Trigger
        // TriggerBuilder 用于构建触发器实例
        CronTrigger cronTrigger = TriggerBuilder.newTrigger().withIdentity("知识点And题目", "推送类型")
                .withSchedule(cronScheduleBuilder).build();
        ...
        if (!scheduler.isShutdown()) {
            ...
            task.setJobName("知识点And题目");
            task.setBeanClass("com.ershiyi.JobDemo.QuartzJobListener");
            task.setTypeName("推送类型");
            runJobNow(task);
            scheduler.start();
        }
    }
    ...
    public void addJob(CronTask task) {
        try {
            ...
            JobDetail jobDetail = JobBuilder.newJob(jobClass).withIdentity(task.getJobName(), task.getTypeName())
                    ...
                    .build();
            //判断map参数是否有值
            if (MapUtils.isNotEmpty(task.getParameterMap())) {
                Set<String> set = task.getParameterMap().keySet();
                if ((task.getParameterMap().keySet())!=null) {
                    for (String key : set) {
