...
public class RedisUtils {
    ...
    public static <T> List<T> getList(String key, long start, long end) {
        try {
            ...
            List<Object> ranges = redisTemplate.opsForList().range(key, start, end);
    ...
    public static <T> List<T> getList(String key) {
        try {
            List<T> list = Lists.newArrayList();
            List<Object> ranges = redisTemplate.opsForList().range(key, 0, -1);
            if(ObjectUtils.isEmpty(ranges)) {
                for (Object range : ranges){
                    list.add((T)range);
                }
