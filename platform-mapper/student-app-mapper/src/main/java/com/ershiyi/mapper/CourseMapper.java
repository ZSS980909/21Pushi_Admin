...
public interface CourseMapper extends AbstractMapper<JHZCourseDTO> {
    ...
    @Select("select id from common_course where  id not in (select courseId from common_course_purchase where studenterId = #{studenterId} and status = 1) and deleted = 0 and grade = #{grade} order by   createdt desc")
    List<Integer> courseForSubjectone(@Param("studenterId")  String studentId,@Param("grade")int grade);
